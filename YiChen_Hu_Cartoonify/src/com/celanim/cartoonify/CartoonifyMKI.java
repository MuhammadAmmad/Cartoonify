...
public class CartoonifyMKI {
	...
	public void gaussianBlurAndSobelEdgeDetect(int threshold) throws CLBuildException, IOException {
		...
		System.out.println(" init took  " + (end - start) / 1e3 + " secs.");
		
		CLQueue queue = context.createDefaultOutOfOrderQueue();
		
		CLBuffer<Integer> memInImage = context.createIntBuffer(CLMem.Usage.Output, tasks);
		Pointer<Integer> imageBuff = memInImage.map(queue, MapFlags.Write);
		CLBuffer<Integer> memGauOut = context.createIntBuffer(CLMem.Usage.Output, tasks);		
		CLBuffer<Integer> memSobOut = context.createIntBuffer(CLMem.Usage.Output, tasks);		
		

		
		start = System.currentTimeMillis();//o
	/*	for (int i = 0; i < tasks; i++) {
			imageBuff.set(i , currentImage()[i]);
		}*/
		...
		System.out.println(" loop took  " + (end - start) / 1e3 + " secs.");
		
		
		start = System.currentTimeMillis();//o
		CLProgram program = context.createProgram(IOUtils.readTextClose(CartoonifyMKI.class.getResourceAsStream("CartoonifyKernelsMKI.cl"))).build();
		end = System.currentTimeMillis();//l
		System.out.println(" build source took  " + (end - start) / 1e3 + " secs.");
		...
		kernelGau.enqueueNDRange(queue, new int[] { (tasks/512+1)*512 },
				new int[] { 512 });
		
		kerneSob.enqueueNDRange(queue, new int[] { (tasks/512+1)*512 },
				new int[] { 512 });		
		queue.finish();

		int[] gauPixels = new int[tasks];
		int[] sobPixels = new int[tasks];
		
		Pointer<Integer> outputGau = memGauOut.read(queue);
		Pointer<Integer> outputSob = memSobOut.read(queue);
		end = System.currentTimeMillis();//l
		System.out.println(" body took  " + (end - start) / 1e3 + " secs.");
		
		start = System.currentTimeMillis();//o
		gauPixels=(int[]) outputGau.getArray();
		sobPixels=(int[]) outputSob.getArray();
		
		


		/*for (int i = 0; i < tasks; i++) {
			gauPixels[i] = outputGau.get(i);
			sobPixels[i] = outputSob.get(i);	
		}*/
		end = System.currentTimeMillis();//l

		System.out.println(" finish took " + (end - start) / 1e3 + " secs.");
		...
		System.out.println(" push image took " + (end - start) / 1e3 + " secs.");
		...
		//Dump the image to global memory
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				imageBuff.set(y * width + x , pixel(x,y));
			}
		}
		//Apply the dump
		memInImage.unmap(queue, imageBuff);
		CLProgram program = context.createProgram(IOUtils.readTextClose(CartoonifyMKI.class.getResourceAsStream("CartoonifyKernelsMKI.cl"))).build();
		CLKernel kernel = program.createKernel("gaussianBlurAndSobelEdgeDetect", memInImage,
				height, width,  memOut);
		
		CLKernel kerne2 = program.createKernel("sobelEdgeDetect", memOut,
				height, width, threshold, memOut);
		
		final long time0 = System.nanoTime();
		kernel.enqueueNDRange(queue, new int[] { (tasks/512+1)*512 },
				new int[] { 512 });
		...
		int[] newPixels = new int[tasks];
		Pointer<Integer> output = memOut.read(queue);
		for (int i = 0; i < tasks; i++) {
			newPixels[i] = output.get(i);
		}
		pushImage(newPixels);*/
	}
	 ...
	 */
	public void sobelEdgeDetect1(int threshold) {
		int[] newPixels = new int[width * height];
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
	...
	public void sobelEdgeDetect(int threshold) throws CLBuildException, IOException {
		final int tasks = width*height;
		System.out.println("work item size="
				+ width*height);
		...
		memInImage.unmap(queue, imageBuff);
		CLProgram program = context.createProgram(IOUtils.readTextClose(CartoonifyMKI.class.getResourceAsStream("CartoonifyKernelsMKI.cl"))).build();
		CLKernel kernel = program.createKernel("sobelEdgeDetect", memInImage,
				height, width, threshold, memOut);
...
//		}
	...
	public static void main(String[] args) throws IOException {
		...
		CartoonifyMKI cartoon = new CartoonifyMKI();
		for (; currArg < args.length; currArg++) {
			...
			long start = time0;
			cartoon.gaussianBlurAndSobelEdgeDetect(threshold);
			long end = System.currentTimeMillis();
			if (debug) {
