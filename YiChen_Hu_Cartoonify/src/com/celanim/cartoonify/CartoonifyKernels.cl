...
{		
	...
	newPixels[get_global_id(0)] = createPixel(red,green,blue); 
}
...
__kernel void reduceColours(__global const int* currentImage, const int height, const int width , const int numPerChannel , __global int* newPixels) 
{		
	int rgb = currentImage[get_global_id(0)];
	int newRed = quantizeColour((rgb>>16) & 255, numPerChannel);
	int newGreen = quantizeColour((rgb>>8) & 255, numPerChannel);
	int newBlue = quantizeColour(rgb & 255, numPerChannel);
	int newRGB = createPixel(newRed, newGreen, newBlue);
	newPixels[get_global_id(0)] = newRGB;
}
